...
# Se ejecuta cada tick desde fun_skyland:zonas/main
...
execute as @e[type=marker,tag=sl.zone.np] at @s run function fun_skyland:zonas/np/run_area_checks_for_marker

# 3. Chequea si la zona es nueva
function fun_skyland:zonas/np/check_new
