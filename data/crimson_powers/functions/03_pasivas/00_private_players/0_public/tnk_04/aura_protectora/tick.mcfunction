...
execute if predicate crimson_powers:sneak run function crimson_powers:03_pasivas/00_private_players/0_public/tnk_04/aura_protectora/g1_tap
...
execute as @a[tag=aura_tank,tag=!ene] if score @s tap_cd matches 300.. run function crimson_powers:03_pasivas/00_private_players/0_public/tnk_04/aura_protectora/apply_periodic_buff

# 3. A los jugadores cuyo contador ha llegado a 15 segundos (300), les quitamos el buff.
execute as @a[scores={tap_g=300..},tag=!ene] run function crimson_powers:03_pasivas/00_private_players/0_public/tnk_04/aura_protectora/off
