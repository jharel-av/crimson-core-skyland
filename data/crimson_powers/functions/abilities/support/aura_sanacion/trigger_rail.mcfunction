...
# ---- On Cooldown ----
execute if score @s cd_aura matches 1.. run function crimson_powers:abilities/support/aura_sanacion/on_cooldown
# ---- Ready to Use ----
execute if score @s cd_aura matches ..0 run function crimson_powers:abilities/support/aura_sanacion/trigger
## --- Kill Item (Ya no usado) ---
# Elimina el item del suelo si fue detectado por un jugador 'sup', para que no sea procesado de nuevo.
# kill @e[distance=0..,nbt={Item:{tag:{sup_aura:1b}}},limit=1,sort=nearest,type=item]
